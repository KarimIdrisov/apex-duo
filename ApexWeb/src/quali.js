...
export function qualiLap(drv, car, track, setup, risk, rng, carMean = 0) {
...
export function buildGrid(field, track, seed) {
  ...
  return field
    .map(f => ({ idx: f.idx, abbrev: f.abbrev, time: qualiLap(f, f.car, track, f.setup, f.risk ?? 0.5, r, carMean) }))
