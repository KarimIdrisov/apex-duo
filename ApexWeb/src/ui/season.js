...
export function render(root, ctx) {
  ...
  const statusChip = d => d.status === "lead" ? `<span style="font-size:10px;background:var(--warn);color:#1a1205;border-radius:4px;padding:1px 6px;font-weight:700">★ 1-й номер</span>` : d.status === "support" ? `<span style="font-size:10px;color:var(--muted);border:1px solid var(--border);border-radius:4px;padding:0 6px">саппорт</span>` : "";
  ...
  const mktCard = d => { const fa = freeAgent(d), cost = signCostAt(d, signLen, clauses), ipRaw = interest(d, meStrength, signLen) + (clauses.lead ? CLAUSE.leadInterest : 0), ip = Math.round(Math.min(0.97, ipRaw) * 100), curTeam = (TEAMS[d.teamIdx] || {}).name || "", buy = buyout(d), can = c.money >= cost;
