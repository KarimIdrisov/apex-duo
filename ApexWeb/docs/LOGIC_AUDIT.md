...
1. **Winner concentration — and its VERIFIED root: driver ≫ car (~24×).** McLaren wins ~80% at default. **Measured** (over the real grid, `node` sensitivity check): the **car contributes only a ±0.065 s/lap spread** to lap time while the **driver-skill spread is ~1.57 s/lap → a 24:1 ratio.**
   ...
   - → **PROPOSED (high value, the real fix):** add an **absolute car-pace term** — e.g. `−CAR_PACE_K·(carPerf − fieldMean)` where `carPerf` blends power+aero — so a better car is genuinely faster. This creates real car-vs-driver tension, makes the R&D/car meta matter, and lets you **dial the car:driver ratio** toward something like ~1:2 (driver still primary, car decisive). Then **compress the driver-skill spread** (1.57 s/lap is large) and re-tune to the §17 corridor. *Diminishing-returns `log` is moot until this term exists — you can't compress a ~0.065 s effect.*
...
13. **Slipstream tow over-powers the pass (VERIFIED).** `passAccrual = (max(0,edge) + tow)·push·(0.5+straightness)` lets the **tow alone** build credit — even at `edge ≤ 0` (equal or slower pace). **Measured** in a brake zone: tow ≈ `SLIP_K(0.25)·straightness(0.8)·power(0.95)` ≈ 0.19/tick → credit ≈ `(0+0.19)·(0.5+0.8)·(0.7+0.6·overtaking)` ≈ 0.29/tick; against `resist ≈ 1.06` that's **~3.7 ticks ≈ 0.9 s → a pass with no pace advantage.** Risk: equal-pace cars swap positions every zone (artificial). **Mitigating context:** the overall pass corridor (27–37/race, ~3 places/car) shows it isn't currently catastrophic — a slower car can't *stay* pinned (the pin only pushes back, never pulls forward), and per-tick noise keeps `edge` off exactly 0 — so it self-limits. Hence **major (realism), not critical.**
   - → **PROPOSED:** make the tow *amplify a real edge* rather than create a pass from nothing — e.g. `tow_eff = tow · clamp(edge/EDGE_REF, 0..1)` or require a small positive `edge` for the tow to convert. **Do NOT** hard-zero the tow at `edge ≤ 0` (a reviewer's suggestion) — that kills the realistic draft pass of an equal car, which *should* be possible, just not in 0.9 s. Tune so a tow pass of a near-equal car takes a few zones (laps), not one. Re-check the §17 overtaking corridor after.
