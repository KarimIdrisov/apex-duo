...
test("a strong qualifier out-qualifies a same-overall racer", () => {
  ...
  let qWins = 0;
  for (let s = 0; s < 100; s++) {
    if (qualiLap(quali, car, TRACK, [0.5,0.5,0.5,0.5,0.5,0.5], 0.3, new RNG(s)) <
        qualiLap(racer, car, TRACK, [0.5,0.5,0.5,0.5,0.5,0.5], 0.3, new RNG(s))) qWins++;
  }
...
test("higher risk lowers the mean lap time but raises variance", () => {
  const safe = [], risky = [];
  for (let s = 0; s < 200; s++) {
    safe.push(qualiLap(drv, car, TRACK, [0.5,0.5,0.5,0.5,0.5,0.5], 0.1, new RNG(s)));
    risky.push(qualiLap(drv, car, TRACK, [0.5,0.5,0.5,0.5,0.5,0.5], 0.9, new RNG(s)));
  }
...
test("a composed driver loses less time to lock-ups under pressure (§18.7)", () => {
  ...
  for (let s = 0; s < 300; s++) {          // high risk → mistakes matter; same seed → same noise, only the lock-up roll differs
    calmSum += qualiLap(calm, car, TRACK, [0.5,0.5,0.5,0.5,0.5,0.5], 0.9, new RNG(s), cm);
    rattSum += qualiLap(rattled, car, TRACK, [0.5,0.5,0.5,0.5,0.5,0.5], 0.9, new RNG(s), cm);
  }
...
test("buildGrid returns all cars sorted fastest-first", () => {
  ...
  const field = TEAMS.flatMap(t => t.drivers.map(d => ({
    idx: idx++, abbrev:d.abbrev, skill:d.skill, car:t.car, setup:[0.5,0.5,0.5,0.5,0.5,0.5], risk:0.5,
  })));
